...
<script>
//conexion, el archivo del select
var selectinfo=[];

</script>

<?php
include('mysql_resources/connection.php');
$sql='select info2 from select_tab';
//echo $sql; 
...
if ($result) {
  ...
  while($row = $result->fetch_row()){
    ...
    if($count%10==0 || $count==$row_cnt){
      ...
      ?>
      <script>
        var sel =<?php echo $selectinfo  ?>;
        if(selectinfo.length==0){
          selectinfo=sel;
          //autocomplete( document.getElementById(inputName),selectinfo);
        }else{
          selectinfo= selectinfo.concat(sel);
          //autocomplete( document.getElementById(inputName),selectinfo);
